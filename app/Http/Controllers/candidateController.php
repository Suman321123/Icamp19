...
class candidateController extends Controller
{
    ...
    function test() {
        ...
        {
            $candidate = new Candidate;
            $candidate->name = $users[$i]->name;
            $candidate->email = $users[$i]->email;
            $password = str_replace("$2a$", "$2y$", $users[$i]->password);
            $candidate->password = $password;
            if($users[$i]->payments_instamojo_id!=NULL)
            {
                $candidate->payment_id = $users[$i]->payments_instamojo_id;
                $candidate->payment_complete = 1;
            }
            else if($users[$i]->payments_rec_no!=NULL)
            {
                $candidate->payment_id = $users[$i]->payments_instamojo_id;
                $candidate->payment_complete = 1;
            }
